...
def encode_urlencode( foo, errors = 'strict' ):
    if isinstance(foo,unicode):
        foo = foo.encode('utf-8')
    outbuf = ''
    for x in foo:
        if ord(x) > 127 or ( not x.isalnum() and x not in '-_.!~*"()\'' ):
            outbuf += '%%%02X' % ( ord(x) )
        else:
            outbuf += x
    return outbuf
...
def encoder( foo, errors='strict' ):
    return encode_urlencode( foo ), len(foo)
def decoder( foo, errors='strict' ):
    return decode_urlencode( foo ), len(foo)
...
def search_function( encoding ):
    if encoding not in ['urlencode','url-encode','url-safe','urlsafe']:
        return None
